...
import { MARKETS_PLUTO_ERAS as eras, type MarketsPlutoEra } from "@/lib/industry/markets-pluto-eras-data";
...
export function catalystTag(catalyst: MarketsPlutoEra["catalysts"][number], era: MarketsPlutoEra): string {
  const tag = catalyst.label ?? (catalyst.year < era.startYear ? "Precursor" : null);
  return `${catalyst.period ?? catalyst.year}${tag ? ` · ${tag}` : ""}`;
}

export default function MarketsPlutoDrawer({
  index,
  onNavigate,
  onClose,
}: {
  ...
  return (
    <>
      ...
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="market-pluto-title"
      ...
      >
        ...
        <div className="relative shrink-0 overflow-hidden border-b border-rule px-6 py-5" style={{ borderTop: `2px solid ${era.color}` }}>
          ...
          <div className="relative flex items-start justify-between gap-4">
            <p className="datum text-xs uppercase tracking-[0.16em]" style={{ color: era.color }}>
              Pluto in {era.sign} · {era.startYear}–{era.endYear}{era.hypothesis ? " · Hypothesis" : ""}
            </p>
            <button
              ref={closeRef}
              type="button"
          ...
          <p className="relative mt-0.5 text-sm text-bone-faint">{era.theme}</p>
          <p className="datum relative mt-4 text-[0.625rem] uppercase tracking-widest text-bone-faint">Capital organized through</p>
          <h2 id="market-pluto-title" className="relative mt-1 text-3xl font-semibold leading-tight" style={{ color: era.color }}>{era.headline}</h2>
          <p className="relative mt-1 text-lg italic text-bone-soft">{era.archetype}</p>
        ...
        <div className="space-y-7 px-6 py-6">
          <section>
            <h3 className="datum mb-2 text-xs uppercase tracking-widest text-bone-faint">How power is organized</h3>
          ...
          <section className="border-l-2 pl-5" style={{ borderColor: era.color }}>
            ...
            {era.manifestations.some((item) => item.layer) ? (
              <dl className="space-y-2">
                {era.manifestations.map((item) => (
                  <div key={item.title} className="grid grid-cols-[5.5rem_1fr] items-baseline gap-3">
                    <dt className="datum text-[0.625rem] uppercase tracking-widest" style={{ color: era.color }}>{item.layer}</dt>
                    <dd className="text-[1.0625rem] leading-relaxed text-bone">{item.title}</dd>
                  </div>
                ))}
              </dl>
            ) : (
              <Bullets items={era.manifestations.map((item) => item.title)} className="text-[1.0625rem] leading-relaxed text-bone" />
            )}
          </section>
          ...
          <section className="border-t border-rule pt-6">
            <h3 className="datum mb-2 text-xs uppercase tracking-widest text-bone-faint">Where power concentrates / breaks down</h3>
            <Bullets items={era.shadow} className="leading-relaxed text-bone-soft" />
          </section>

          {/* Each Pluto era is a reorganization of the previous one, so the hand-off gets the most weight */}
          <section
          ...
          >
            <h3 className="datum text-xs uppercase tracking-widest text-bone-faint">Reorganization →</h3>
            <p className="mt-2 text-xl font-semibold leading-tight">
              <span style={{ color: era.color }}>{era.archetype}</span>
              <span className="text-bone-faint"> → </span>
              <span style={{ color: next?.color ?? "var(--color-bone-faint)" }}>{next?.archetype ?? "?"}</span>
            </p>
